      ...
      {open && (
        ...
        >
          <div className="queue-head">
            <div>
              <h2>Queue</h2>
              {context && <div className="dim" style={{ fontSize: 13 }}>From {context.title}</div>}
            </div>
          ...
          <div className="queue-body">
                  ...
                  {upcoming.map((song, k) => {
                    ...
                    return (
                      <motion.div
                        ...
                        transition={{ duration: 0.22, delay: Math.min(k * 0.02, 0.3) }}
                      >
            ...
            {history.length > 0 && (
              <>
                ...
                {history.map((song, i) => (
                  <div key={`${song.id}-${i}`} className="queue-item" onClick={() => jumpTo(i)} style={{ opacity: 0.6 }}>
                    <Img src={song.image} alt="" />
                    <div style={{ minWidth: 0 }}>
                      <div className="t truncate">{song.title}</div>
                      <div className="s truncate">{song.artistNames || song.subtitle}</div>
